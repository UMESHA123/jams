...
class FacultyLogInPage():
    def __init__(self,root):
        ...
        self.image=ImageTk.PhotoImage(file='All_Program_Photos/faculty_log_in_page.png')
        ...
        forget_button=Button(self.root,text='Forget your password.',font=('Bookman Old Style',10),bd=0,bg='#1E1818',fg='red',width=20,command=self.forget_your_password,activebackground='#1E1818')
        forget_button.place(x=520,y=455)

        login_button=Button(self.root,text='Log in.',bd=0,bg='#FFC312',fg='black',font=('times new roman',12,'bold'),width=20,command=self.sigin_confirm_data,activebackground='#FFC312')
        login_button.place(x=578,y=410)
                    ...
                    if self.var_user_confirm_password.get()=='':
                        ...
                        if self.var_user_password_sing_up.get()!=self.var_user_confirm_password.get():
                            ...
                            if self.sing_upotp!=self.var_user_email_otp_sing_up.get():
                                messagebox.showerror('Error','Enter the correct otp\nThe otp is send to your email.\nplease check it')
                            else:
                                cur.execute('insert into faculty_sing_up_data (user_email,otp,user_password,user_confirm_password) values(?,?,?,?)',(
        ...
        if self.var_user_email.get()=='':
            ...
            if self.var_pass_word.get()=='':
                ...
                if self.var_otp.get()=='':
                    ...
                    cur.execute('select user_password from faculty_sing_up_data where user_email=?',(self.var_user_email.get(),))
                    ...
                    if login_user_password==None:
                        ...
                        if login_user_password[0]!=self.var_pass_word.get():
                            ...
                            if self.otp==self.var_otp.get():
                                messagebox.showinfo('Info','OTP sre correct',parent=self.root)
                             ...
                             ###########################################################################################################################
                                ...
                                messagebox.showerror('Error','chech you OTP and password',parent=self.root)
                    
if __name__=='__main__':
    root=tk.Tk()
    ob1=FacultyLogInPage(root)
    root.mainloop()