...
import smtplib
import random
from nes_main_page import NesMainPage
import time 
import psycopg2
class NesLogInPage():
    def __init__(self,root):
        ...
        self.var_check_otp=StringVar()
        self.sing_upotp=StringVar()
        ################################################################################

        frame=Frame(self.root)
        frame.pack(fill=BOTH,expand=1)

        self.image=ImageTk.PhotoImage(file='All_Program_Photos/nes_log_in_page.png')
        label=Label(frame,image=self.image)
        label.pack(fill=BOTH,expand=1)

        user_email=ttk.Entry(self.root,width=30,textvariable=self.var_user_email)
        user_email.place(x=570,y=267)

        password_entry=ttk.Entry(self.root,width=30,textvariabl=self.var_pass_word)
        password_entry.place(x=570,y=369)#319

        otp_entry=ttk.Entry(self.root,width=30,textvariabl=self.var_otp)
        otp_entry.place(x=570,y=319)#369
                    ...
                    if self.var_user_confirm_password.get()=='':
                        ...
                        if self.var_user_password_sing_up.get()!=self.var_user_confirm_password.get():
                            ...
                            if self.sing_upotp!=self.var_user_email_otp_sing_up.get():
                                messagebox.showerror('Error','The otp is not correct please check it\nthe otp is send to your email',parent=self.forget_password)
                            else:

                                cur.execute('insert into nes_sing_up_data (user_email,otp,user_password,user_confirm_password) values(%s,%s,%s,%s)',(
        ...
        if self.var_user_email.get()=='':
            ...
            if self.var_pass_word.get()=='':
                ...
                if self.var_otp.get()=='':
                    ...
                    cur.execute('select user_password from nes_sing_up_data where user_email=%s',(self.var_user_email.get(),))
                    login_user_password=cur.fetchone()
                    print(login_user_password[0])
                    ...
                    if login_user_password==None:
                        ...
                        if login_user_password[0]!=self.var_pass_word.get():
                            ...
                            messagebox.showerror('Error','your password is incorrect',parent=self.root)
                        else:
                            ...
                            if self.otp==self.var_otp.get():
                                ...
                                self.ob1=NesMainPage(self.sigin_cinfirm)
                                ###############################################################################################################################
                            else:
                                messagebox.showerror('Error','your OTP is not currect',parent=self.root)           
                    
if __name__=='__main__':
    root=tk.Tk()
    ob1=NesLogInPage(root)
    root.mainloop()