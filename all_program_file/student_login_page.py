...
class StudentLogInPage():
    def __init__(self,root):
        ...
        self.image=ImageTk.PhotoImage(file='All_Program_Photos/student_log_in_page.png')
        label=Label(frame,image=self.image,bg='black')
        label.pack(fill=BOTH,expand=1)

        user_email=ttk.Entry(self.root,width=34,textvariable=self.var_user_email)
        user_email.place(x=569,y=260)

        password_entry=ttk.Entry(self.root,width=34,textvariabl=self.var_pass_word)
        password_entry.place(x=569,y=360)#310

        singin_otp_entry=ttk.Entry(self.root,width=34,textvariabl=self.var_otp)
        singin_otp_entry.place(x=569,y=310)#360
        ...
        forget_button.place(x=520,y=445)
        ...
        login_button.place(x=576,y=400)
    ...
    def forget_your_password(self):
        ...
        user_email1=ttk.Entry(frame1,width=32,textvariable=self.var_user_email_sing_up)
        user_email1.place(x=565,y=260)

        otp_email1=ttk.Entry(frame1,width=32,textvariable=self.var_user_email_otp_sing_up)
        otp_email1.place(x=565,y=310)

        password=ttk.Entry(frame1,width=32,textvariable=self.var_user_password_sing_up)
        password.place(x=565,y=359)

        confirm=ttk.Entry(frame1,width=32,textvariable=self.var_user_confirm_password)
        confirm.place(x=565,y=410)
    ...
    def singup_send_otp(self):
        ...
        if self.var_user_email_sing_up.get()=="":
            messagebox.showerror('Error','User Email must be required.',parent=self.forget_password)
        else:
            ...
            chk=ss.ehlo()
            if chk[0]==250:
                messagebox.showinfo('Info','Email.is send to your email.\n plese check it.',parent=self.forget_password)
                ...
                if self.var_user_password_sing_up.get()=='':
                    ...
                    if self.var_user_confirm_password.get()=='':
                        ...
                        if self.var_user_password_sing_up.get()!=self.var_user_confirm_password.get():
                            ...
                            cur.execute('insert into student_sing_up_data (user_email,otp,user_password,user_confirm_password) values(%s,%s,%s,%s)',(
        ...
        if self.var_user_email.get()=='':
            ...
            if self.var_pass_word.get()=='':
                ...
                if self.var_otp.get()=='':
                    ...
                    cur.execute('select user_password from student_sing_up_data where user_email=%s',(self.var_user_email.get(),))
                    ...
                    if login_user_password==None:
                        ...
                        if login_user_password[0]!=self.var_pass_word.get():
                            ...
                            if self.otp==self.var_otp.get():
                                messagebox.showinfo('Info','OTP is currect',parent=self.root)
                                
                                
                                ###########################################################################################################################
                                self.sigin_cinfirm=Toplevel(self.root)
                                self.ob1=StudentMainPage(self.sigin_cinfirm)
                                ###############################################################################################################################
                                
                            else:
                                messagebox.showerror('Error','Check your OTP \n',parent=self.root)
                

if __name__=='__main__':
    root=tk.Tk()
    ob1=StudentLogInPage(root)
    root.mainloop()